 ...
 *
 * To access the underlying [T], using [withLock] is preferred; if that is not possible, use [lock] or [tryLock],
 * then manually [unlock] afterward. A typical usage pattern is:
 *
 * ```
...
interface Sync<T : Any> {
    ...
    /**
     * Locks the resource if it is unlocked, or blocks until a lock is acquired, returning the [T] once obtained.
     *
    ...
    /**
     * Attempts to acquire the lock right now if it is unlocked. If successful, returns the [T]; otherwise if the lock
     * was already obtained, returns null.
     *
     * This must be paired with an [unlock] call later.
     */
    fun tryLock(): T?
    ...
    /**
     * Attempts to acquire the lock for the next duration provided. If successful, returns the [T]; otherwise if the lock
     * was already obtained, returns null.
     *
     * This must be paired with an [unlock] call later.
     */
    ...
    /**
     * Acquires the lock, blocking if it is already locked, and calls the function provided with the [T].
     * This method handles locking and unlocking.
     */
