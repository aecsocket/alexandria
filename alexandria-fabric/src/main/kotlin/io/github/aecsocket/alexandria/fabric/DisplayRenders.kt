...
fun ServerPlayer.packetReceiver() = PacketReceiver { connection.send(it) }
...
sealed class DisplayRender(
  ...
  override fun spawn(position: DVec3) =
      this.also {
        receiver.send(
  ...
  override fun despawn() = this.also { receiver.send(ClientboundRemoveEntitiesPacket(eid)) }

  override fun position(value: DVec3) =
      this.also { receiver.send(teleportEntityPacket(eid, value.x, value.y, value.z)) }

  override fun transform(value: FAffine3) =
      this.also {
        receiver.send(
            ClientboundSetEntityDataPacket(
                ...
                listOf(
                    SynchedEntityData.DataValue(
                        TRANSLATION, EntityDataSerializers.VECTOR3, value.translation.toVector3f()),
                    SynchedEntityData.DataValue(
                        SCALE, EntityDataSerializers.VECTOR3, value.scale.toVector3f()),
                    SynchedEntityData.DataValue(
                        LEFT_ROTATION,
  ...
  override fun interpolationDelay(value: Int) =
      this.also {
        ...
        receiver.send(
            ClientboundSetEntityDataPacket(
                ...
                listOf(
                    SynchedEntityData.DataValue(
                        INTERPOLATION_DELAY, EntityDataSerializers.INT, value),
                )))
      }

  override fun interpolationDuration(value: Int) =
      this.also {
        require(value >= 0) { "requires value >= 0" }
        receiver.send(
            ClientboundSetEntityDataPacket(
                ...
                listOf(
                    SynchedEntityData.DataValue(
                        INTERPOLATION_DURATION, EntityDataSerializers.INT, value),
                )))
      }

  override fun billboard(value: Billboard) =
      this.also {
        receiver.send(
            ClientboundSetEntityDataPacket(
                eid,
  ...
  override fun viewRange(value: Float) =
      this.also {
  ...
  override fun cullBox(value: FVec2) =
      this.also {
  ...
  override fun glowing(value: Boolean) =
      this.also {
        receiver.send(
            ClientboundSetEntityDataPacket(
                eid,
                listOf(
                    SynchedEntityData.DataValue(
                        ENTITY_FLAGS,
                        EntityDataSerializers.BYTE,
                        (if (value) 0x40 else 0).toByte()),
                )))
      }

  override fun glowColor(value: TextColor) =
      this.also {
        receiver.send(
            ClientboundSetEntityDataPacket(
                eid,
                listOf(
                    SynchedEntityData.DataValue(
                        GLOW_COLOR_OVERRIDE, EntityDataSerializers.INT, value.value()),
                )))
      }
}
      ...
      this.also {
        receiver.send(
            ClientboundSetEntityDataPacket(
                ...
                listOf(
                    SynchedEntityData.DataValue(
                        TEXT, EntityDataSerializers.COMPONENT, audiences.toNative(value)),
                )))
      }
      ...
      this.also {
        receiver.send(
            ClientboundSetEntityDataPacket(
                ...
                listOf(
                    SynchedEntityData.DataValue(
                        BACKGROUND_COLOR, EntityDataSerializers.INT, argbToInt(value)),
                )))
      }
