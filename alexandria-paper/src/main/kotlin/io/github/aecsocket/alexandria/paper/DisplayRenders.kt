...
sealed class DisplayRender(
  ...
  override fun despawn() = this.also { receiver.send(WrapperPlayServerDestroyEntities(eid)) }
            ...
            WrapperPlayServerEntityMetadata(
                ...
                listOf(
                    EntityData(
                        ...
                        value.translation.run { Vector3f(x, y, z) }),
                    EntityData(
                        SCALE, EntityDataTypes.VECTOR3F, value.scale.run { Vector3f(x, y, z) }),
                    EntityData(
                        LEFT_ROTATION,
                        EntityDataTypes.QUATERNION,
                        value.rotation.run { Quaternion4f(x, y, z, w) }),
                )))
      }
      ...
      this.also {
        receiver.send(
            WrapperPlayServerEntityMetadata(
                ...
                listOf(
                    EntityData(
                        ENTITY_FLAGS, EntityDataTypes.BYTE, (if (value) 0x40 else 0).toByte()),
                )))
      }
