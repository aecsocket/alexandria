package com.github.aecsocket.alexandria.paper
...
private const val BUKKIT_NBT_KEY = "PublicBukkitValues"
...
sealed interface ServerElement {
    ...
    data class OfStack(
        ...
        override fun toString(): String {
            val name = plainText(stack.itemMeta?.displayName() ?: translatable(stack.translationKey()))
...
class ElementResolver(
    ...
    interface ElementsResolved {
        val possible: Int
        val marked: Int
    }

    private data class ElementsResolvedImpl(
        override var possible: Int = 0,
        override var marked: Int = 0
    ) : ElementsResolved

    fun resolve(): Map<ElementType, ElementsResolved> = Operation().let {
        it.resolve()
        it.resolved
    }

    private inner class Operation {
        val resolved = EnumMap<ElementType, ElementsResolvedImpl>(ElementType::class.java).apply {
            ElementType.values().forEach { put(it, ElementsResolvedImpl()) }
        }

        fun applyWorld(world: World) {
            val resolved = resolved[ElementType.WORLD]!!
            resolved.possible++
        ...
        fun applyChunk(chunk: LevelChunk) {
            val resolved = resolved[ElementType.CHUNK]!!
            resolved.possible++
        ...
        fun applyEntity(entity: Entity) {
            val resolved = resolved[ElementType.ENTITY]!!
            resolved.possible++
        ...
        fun marked(tag: CompoundTag) =
            tag.contains(BUKKIT_NBT_KEY) && (tag.get(BUKKIT_NBT_KEY) as CompoundTag).contains(markerStr)

        fun applyStack(stack: ItemStack, holder: StackHolder) {
            if (stack.isEmpty)
                return
            val resolved = resolved[ElementType.STACK]!!
            resolved.possible++

            val element = ServerElement.OfStack(stack.bukkitStack, holder)
            stack.tag?.let { tag ->
                if (marked(tag)) {
                    callback(element)
                    resolved.marked++
            ...
            if (item is BlockItem) {
                ...
                if (item.block is ShulkerBoxBlock) {
                    BlockItem.getBlockEntityData(stack)?.let { tag ->
                        if (tag.contains("Items", 9)) {
                            tag.getList("Items", 10).forEachIndexed { idx, itemTag ->
                                resolved.possible++
                                ...
                                if (itemTag.contains(TAG) && marked(itemTag.get(TAG) as CompoundTag)) {
                                    callback(ServerElement.OfStack(ItemStack.of(itemTag).bukkitStack, StackHolder.ByShulkerBox(
                                        element, idx
                                    )))
                                    resolved.marked++
                                }
        ...
        fun applyBlock(world: World, block: BlockEntity) {
            val resolved = resolved[ElementType.BLOCK]!!
            resolved.possible++
            ...
            Bukkit.getServer().worlds.forEach { world ->
                ...
                world.handle.level.chunkSource.chunkMap.updatingChunks.visibleMap.forEach { (_, holder) ->
                    ...
                    holder.fullChunkNow?.let { chunk ->
                        ...
                        chunk.bukkitChunk.tileEntities
